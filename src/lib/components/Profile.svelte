<script lang="ts">
	import Header from '$lib/components/Header.svelte'; // @ts-ignore: exists
...
{#if $page.data.session?.user?.image && $page.data.session?.user?.name}
	...
	<p class="mx-3 !mt-[-2px] leading-[1.125] inline">
		Connected&nbsp;to&nbsp;Strava&nbsp;as<br /><b
			>{$page.data.session.user.name.replace(/ /g, '\xa0')}</b
		>
	</p>
	<span class="border-l border-[1.5px] h-10 ml-2 mr-3"></span>
	<Button
		class="py-[6px] px-[10px] bg-background hover:bg-card dark:hover:bg-muted border flex items-center h-10 my-3 px-4"
		on:click={() => {
			localStorage.removeItem('allGeojsonFeatures');
			signOut('strava');
		}}
	>
		<p class="font-bold text-l inline text-black dark:text-white truncate">
			Disconnect from
